...
public class VersionManagementUtil {
	 ...
	 *         -1
	 */
	public static int VersionComparison(String versionServer,
			String versionLocal) {
		...
		while (index1 < version1.length() && index2 < version2.length()) {
			int[] number1 = getValue(version1, index1);
			 Logger.i(TAG," ===== number1 ====" + Arrays.toString(number1));  
			int[] number2 = getValue(version2, index2);
			Logger.i(TAG," ===== number2 ====" + Arrays.toString(number2));  
			if (number1[0] < number2[0]) {
				Logger.i(TAG," ===== number2[0] ====" + number2[0]);  
				return -1;
			} else if (number1[0] > number2[0]) {
				Logger.i(TAG," ===== number1[0] ====" + number1[0]);  
				Logger.i(TAG," ===== number2[0] ====" + number2[0]);  
				return 1;
			} else {
				index1 = number1[1] + 1;
