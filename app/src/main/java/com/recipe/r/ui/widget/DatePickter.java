 ...
 * 功能描述：时间选择器
 **/
public class DatePickter extends PopupWindow implements View.OnClickListener {
    ...
    private static OnDateResultListener listener_result;
     ...
     * @param btn_size  确认取消按钮大小
     * @param text_size 选择框中文字大小
     * @param datetime  当前默认选中时间
     *                  <p>
     *                  返回DatePickter获取选中结果
     */
    public static DatePickter showDataPop(final Activity context, int mintime, int maxtime, int btn_size, int text_size, String datetime) {
        OnDatePickedListener listener = new OnDatePickedListener() {
            @Override
            public void onDatePickCompleted(int year, int month, int day, String dateDesc) {
                listener_result.getDateResult(year, month, day, dateDesc);
            }
        };
        DatePickter pickerPopWin = new DatePickter.Builder(context, listener).textConfirm(context.getResources().getString(R.string.str_confirm)) //text of confirm button
                .textCancel(context.getResources().getString(R.string.str_cancel)) //text of cancel button
                .btnTextSize(btn_size) // button text size
                .viewTextSize(text_size) // pick view text size
                .colorCancel(Color.parseColor("#999999")) //color of cancel button
                .colorConfirm(Color.parseColor("#009900"))//color of confirm button
                .minYear(mintime) //min year in loop
                .maxYear(maxtime) // max year in loop
     ...
     * 对外开放获取选中结果的接口
     *
     * @param listener_result
     */
    public void setDateResultListener(OnDateResultListener listener_result) {
    ...
    public interface OnDateResultListener {
        void getDateResult(int year, int month, int day, String dateDesc);
    }
    ...
    public static class Builder {
        ...
        public DatePickter build() {
            if (minYear > maxYear) {
                throw new IllegalArgumentException();
            }
            return new DatePickter(this);
        }
    ...
    public DatePickter(Builder builder) {
...
//        dayLoopView.setTextSize(25);
    ...
    private void initPickerViews() {
        ...
        for (int i = 0; i < yearCount; i++) {
            yearList.add(format2LenStr(minYear + i) + mContext.getResources().getString(R.string.str_year));
        }

        for (int j = 0; j < 12; j++) {
            monthList.add(format2LenStr(j + 1) + mContext.getResources().getString(R.string.str_month));
        }
    ...
    private void initDayPickerView() {
        ...
        for (int i = 0; i < dayMaxInMonth; i++) {
            dayList.add(format2LenStr(i + 1) + mContext.getResources().getString(R.string.str_date));
        }
